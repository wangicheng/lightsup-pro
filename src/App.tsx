...
function App() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center p-6 text-zinc-100 font-sans selection:bg-amber-500/30">
              ...
              <div key={rIndex} className="flex gap-3">
                {row.map((isOn, cIndex) => (
                  <button
                    ...
                    `}
                  >
                    {/* Inner detail for the "Off" state to look like a bulb socket */}
                    {!isOn && (
                      <div className="absolute inset-0 m-auto w-3 h-3 rounded-full bg-zinc-900/50" />
                    )}
                    
                    {/* Shine reflection for the "On" state */}
                    {isOn && (
